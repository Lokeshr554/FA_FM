...
// No APARs installed.
if (egl.eze$$userLibs) egl.eze$$userLibs.push('com.ibm.egl.rui.infobus.InfoBus');
else egl.eze$$userLibs = ['com.ibm.egl.rui.infobus.InfoBus'];
egl.defineRUILibrary( 'com.ibm.egl.rui.infobus', 'InfoBus',
	{
		'eze$$fileName': '/com.ibm.egl.rui_4.3.7/EGLSource/com/ibm/egl/rui/infobus/InfoBus.egl',
		...
		"constructor" : function()
		{
			...
			try { egl.enter("<init>",this,arguments,eze$$updater);
				...
				egl.atLine(this.eze$$fileName,21,465,28, this);
		...
		"subscribe": function( name, callback, eze$$callback$func_, eze$$caller ) {
			...
			try { egl.enter("subscribe",this,arguments,eze$$updater);
				var $result = null;
				egl.addLocalFunctionVariable("name", name, "string", "name");
				egl.addLocalFunctionVariable("callback", callback, "com.ibm.egl.rui.infobus.InfoBusCallback", "callback");
				egl.atLine(this.eze$$fileName,28,767,32, this);
				var ezert$$1 = null;
				ezert$$1 = egl.checkNull(this.bridge, "bridge").subscribe(name, callback, function(eze$$inout$callback) {
					callback = eze$$inout$callback;
					egl.setLocalFunctionVariable("callback", callback, "com.ibm.egl.rui.infobus.InfoBusCallback");
					eze$$callback$func_.call(eze$$caller,callback);
				}, this );
				egl.setLocalFunctionVariable("callback", callback, "com.ibm.egl.rui.infobus.InfoBusCallback");
				egl.atLine(this.eze$$fileName,28,759,42, this);
				$result = ezert$$1;
		...
		"unsubscribe": function( subscription ) {
			...
			try { egl.enter("unsubscribe",this,arguments,eze$$updater);
				egl.addLocalFunctionVariable("subscription", subscription, "any", "subscription");
				egl.atLine(this.eze$$fileName,35,928,33, this);
				egl.checkNull(this.bridge, "bridge").unsubscribe(subscription );
				egl.atLine(this.eze$$fileName,34,880,90, this);
				{ if (!egl.debugg) egl.leave(); }
			} finally {
				if (!egl.debugg){
				} else { egl.leave(); }
		...
		"publish": function( name, data ) {
			...
			try { egl.enter("publish",this,arguments,eze$$updater);
				...
				egl.atLine(this.eze$$fileName,43,1170,27, this);
				egl.checkNull(this.bridge, "bridge").publish(name, data );
				egl.atLine(this.eze$$fileName,42,1118,88, this);
				{ if (!egl.debugg) egl.leave(); }
			} finally {
				if (!egl.debugg){
				} else { egl.leave(); }
		...
		"eze$$getChildVariables": function() {
			...
			{name: "bridge", value : eze$$parent.bridge, type : "com.ibm.egl.rui.infobus.OpenAjaxHubBridge", jsName : "bridge"}
			];
		}
	}
);
