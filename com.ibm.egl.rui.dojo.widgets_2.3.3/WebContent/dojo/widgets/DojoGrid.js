	...
	'div',
{
	...
	"createDojoWidget" : function(parent) {
	    ...
	    if (widgetOrienattion.toLowerCase() == 'rtl')
    			...
    			if(items && items.length>0 && eglWidget.data && eglWidget.data.length > 0){
    				for(var i=0; i<items.length; i++){
    					for(var col=0; col<eglWidget.columns.length; col++){
    						eglWidget.data[i][eglWidget.columns[col].name] = items[i][eglWidget.columns[col].name][0];
    					}
    				}
    			}
    		},
		...
		for (var row=0; row < rowNum; row++) {
		    ...
		    for (var column=0; column < columns.length; column++) {
		    	...
		    	if(data.length <= 0)
		    		...
		    		value = egl.unboxAny(data[row][columns[column].field]);		    		
			    	if (value === undefined){
			    		value = "";
			    	}
		    		if (isVisual || isReverseDirection)
		    			value = this.setBiDiMarkersStr(value,isVisual,isReverseDirection);
		    	} 				    	
    ...
    "createColumns" : function() {
	    ...
	    if (!this.columns.length && this.data.length) {
			...
			for (var n=0; n < this.columns.length; n++) {	    	
				...
				var isVisual = this.getTextLayout() == "Visual";
				var isReverseDirection = this.getReverseTextDirection() == "Yes";
				name = this.setBiDiMarkersStr(name,isVisual,isReverseDirection);
