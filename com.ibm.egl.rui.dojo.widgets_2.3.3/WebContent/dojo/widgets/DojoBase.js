	...
	'egl.ui.rui', 'Widget',
{
	...
	"getOnWidgetLoad" : function() {
		...
		setTimeout(function() {
			dojo.addOnLoad(eglWidget.dojoWidget, function() {		
				eglWidget.handleEvent(eglWidget.onWidgetLoad,"onWidgetLoad",null);
			});
		}, 1);
	...
	"renderWhenDojoIsDoneLoading" : function() {
		//Do not render UI for parent class
	...
	"copyStyle" : function(style) {
		//Old IE does not have length
		if(!style.length || style.length>0){
			var cssText = this.eze$$DOMElement.style.cssText;
			if(egl.IE && egl.IEVersion<9 && cssText){
				cssText = cssText + ';';
			}			
			this.eze$$DOMElement.style.cssText = cssText + style.cssText;	
		}		
	...
	"copyEvents" : function() {
		for(var i=this._events ? this._events.length -1 : -1; i>=0; i--){
			var event = this._events[i];
			dojo.connect( event[0] ? this.dojoWidget : this.dojoWidget.domNode, "on" + (event[0] || event[1]), event[2]);
		}
	},
	"resetWidget" : function(){
		for(var e in  egl._dojoEventNames){
			var event = egl._dojoEventNames[e][1];
	...
	"installEventHandlers" : function(thisObj,ename,handlers) {	
		...
		if(ename === 'change'){
			setTimeout(function() {
				eglWidget._setEvent('change', 'Change', 'Change' );
			}, 100);			
			return handlers || [];	
		}
	...
	"destroy" : function(preserveDom){
		...
		try { this.dojoWidget.destroy(!!preserveDom); } catch(e) { }
	...
	"setTabIndex" : function(tabIndex){
		this._setProperty("tabIndex", "tabIndex", tabIndex);
	},
	"getTabIndex" : function() {
		return this._getProperty("tabIndex", "tabIndex");
	},
	"focus" : function(){
		if(this.dojoWidget){
	...
	"setBiDiMarkersStr" : function(text, isTextTypeVisual, isTextOrientationRightToLeft) {
  	...
  	//Only run once when installing event at the first creation
	...
	"_setEventWithFunc" : function(htmlEventName, eglEventName, dojoEventName, func){
		this._events = this._events || [];
		this._events.push([dojoEventName, htmlEventName, func]);
	    if(this.dojoWidget){
	    	dojo.connect( dojoEventName ? this.dojoWidget : this.dojoWidget.domNode, "on" + (dojoEventName || htmlEventName), func);
	    }
	},
  	...
  	"_isMVCView" : function(){
  		return !!this.getAttribute("isMVCView");
  	}
});
